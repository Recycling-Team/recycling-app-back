...
public class GetItems {
    ...
    public HttpResponseMessage run(
            ...
            @SQLInput(
                name = "items",
                commandText = "SELECT * FROM dbo.items INNER JOIN (SELECT * FROM dbo.users) hlo ON dbo.items.[user] = hlo.user_id WHERE dbo.items.available = 1 AND dbo.items.visible = 1",
                commandType = "Text",
